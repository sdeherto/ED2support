...
ForestType <- "Oldgrowth"  # or "Regrowth"
...
variables <- c("agb","agb_pft", "gpp", "gpp_pft", "npp", "npp_pft", "transp", "transp_pft", "lai", "lai_pft", "biomass", "biomass_pft",
...
for (var in variables) {
    ...
    if (substr(var, nchar(var) - 3, nchar(var)) == "_pft") {
      pft_flag <- 1
      nc_filename <- sprintf("%s_%s_%s_%d_%s.nc", Model, Sitename, ForestType, Age, var)
