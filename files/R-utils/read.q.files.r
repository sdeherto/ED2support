...
read.q.files <<- function( datum
   ...
   for (m in seq(from=tresume,to=ntimes,by=1)){
      ...
      #print(idx)
      #print(length(mymont$NPATCHES.GLOBAL))
      #print(sequence(mymont$NPATCHES.GLOBAL))
      if (length(idx) != length(sequence(mymont$NPATCHES.GLOBAL))) {idx <- seq_len(length(sequence(mymont$NPATCHES.GLOBAL)))}
      #print(idx)
      #------------------------------------------------------------------------------------#
...
#                             browser()
	 ...
	 patch$lai           [[plab]][idx     ] = lai.pa
         ...
         #print(patch$soil.resp [[plab]])
	 #print(root.resp.pa)
	 # Ensure root.resp.pa is the same length as idx
         #print(length(idx))
         if (length(root.resp.pa) != length(sequence(mymont$NPATCHES.GLOBAL))) {root.resp.pa <- rep(root.resp.pa, length.out = length(idx))}
	 #print(root.resp.pa)
	 patch$soil.resp     [[plab]][idx] = patch$soil.resp [[plab]][idx] + root.resp.pa
         ...
         #------ Soil respiration mixes cohort (root) and patch (hetetrophic). ------------#
	 if (length(q.root.resp.pa) != length(sequence(mymont$NPATCHES.GLOBAL))) {q.root.resp.pa <- rep(q.root.resp.pa, length.out = length(idx))}
         #print(q.root.resp.pa)
      ...
      #print(ltor.pa)
      #print(bleafareapa)
      if (length(ltor.pa) != length(sequence(mymont$NPATCHES.GLOBAL))) {ltor.pa <- rep(ltor.pa, length.out = length(idx))}
      ...
      if (length(agvolume.pa) != length(sequence(mymont$NPATCHES.GLOBAL))) {agvolume.pa <- rep(agvolume.pa, length.out = length(idx))}
